...
private:
    ...
    /**
     * Destroys all dynamically allocated memory associated with the
     * current subtree of the quadtree. Recursive helper function.
    ...
    /**
     * Copies the parameter other subtree into the current subtree.
     * Does not free any memory. A recursive helper function.
     * @param other The other subtree's root.
     * @param subroot The quadtree's subtree's root.
     */
    void copyHelper(Node*& other, Node* subroot);
     ...
     * @param img The PNG to be rendered on.
     ...
     * @param subroot The current subroot in the recursion.
     */
    void pruneHelper(const int tol, Node* subroot);

    /*
     * Recursive helper function for pruneSize.
     * @param subroot The current subroot in the recursion.
     */
    int pruneSizeHelper(const int tol, Node* subroot) const;
